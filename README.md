...
> 🎬 PurpleMoon 是一个开箱即用的、跨平台的影视聚合播放器。它基于 **Next.js 14** + **Tailwind&nbsp;CSS** + **TypeScript** 构建，支持多资源搜索、在线播放、收藏同步、播放记录、本地/云端存储，让你可以随时随地畅享海量免费影视内容。
...
2. 进入刚创建的数据库，点击左上角的 Explore Data，将[D1 初始化1](D1初始化1.md) [D1 初始化2](D1初始化2.md)中的内容粘贴到 Query 窗口后点击 **Run All**，等待运行完成
...
```bash
# 拉取预构建镜像
docker pull ghcr.io/purplemoon/PurpleMoon:latest

# 运行容器
# -d: 后台运行  -p: 映射端口 3000 -> 3000
docker run -d --name PurpleMoon -p 3000:3000 ghcr.io/purplemoon/PurpleMoon:latest
```
...
services:
  PurpleMoon:
    image: ghcr.io/purplemoon/PurpleMoon:latest
...
services:
  PurpleMoon-core:
    ...
    environment:
      ...
      - REDIS_URL=redis://PurpleMoon-redis:6379
  ...
  PurpleMoon-redis:
    image: redis
    container_name: PurpleMoon-redis
    restart: unless-stopped
    networks:
      - PurpleMoon-network
    # 如需持久化
...
| AUTH_SECRET                 | 环境变量隔离（安全）                                            | 建议任意32以上位字符串                    | 空                                                                                                                                                                                                       
| CRON_SECRET                 | 为 Cron 任务加上密钥校验 (安全)                                       | 建议任意32以上位字符串                     | 空                  
...
PurpleMoon 支持标准的苹果 CMS V10 API 格式。
