...
function PlayPageClient() {
    ...
    if (artPlayerRef.current) {
      try {
        ...
        if (video) {
          ...
          // 🛑 移动端致命补丁 1：先静音并归零，瞬间切断锁屏音频
          video.muted = true;
          video.currentTime = 0;
          video.pause();

          // 🛑 移动端致命补丁 2：清空 Safari 原生 HLS 的嵌套节点
          ...
          // ==========================================
          // 🛑 移动端致命补丁 3：暴力拔除 DOM 节点
          // 不要等 React 虚拟 DOM 或 ArtPlayer 来慢吞吞地销毁！
          // 在系统回过神来之前，直接手动从真实 DOM 树上斩断引用
          // ==========================================
          if (video.parentNode) {
        ...
        console.debug('✅ [移动端优化版] 播放器资源及DOM节点已彻底拔除');
      } catch (err) {
        console.warn('⚠️ 清理播放器时出现异常，已强制重置引用:', err);
        artPlayerRef.current = null;
      }
    }
  };
  ...
  const handleSkipConfigChange = async (newConfig: {
    ...
    try {
      ...
      if (artPlayerRef.current && artPlayerRef.current.setting) {
        ...
        artPlayerRef.current.setting.update({
          name: '设置片头',
          tooltip: newConfig.intro_time > 0 ? formatTime(newConfig.intro_time) : '设置片头时间',
        });
        artPlayerRef.current.setting.update({
          name: '设置片尾',
          tooltip: newConfig.outro_time < 0 ? `-${formatTime(-newConfig.outro_time)}` : '设置片尾时间',
        });
      }
    ...
    const fetchSourcesData = async (query: string): Promise<SearchResult[]> => {
      try {
        ...
        const results = data.results.filter(
          (result: SearchResult) =>
            result.title.replaceAll(' ', '').toLowerCase() === videoTitleRef.current.replaceAll(' ', '').toLowerCase() &&
            (videoYearRef.current ? result.year.toLowerCase() === videoYearRef.current.toLowerCase() : true) &&
            (searchType ? (searchType === 'tv' && result.episodes.length > 1) || (searchType === 'movie' && result.episodes.length === 1) : true)
        );
  ...
  useEffect(() => {
    const initFromHistory = async () => {
      ...
      try {
        ...
        if (record) {
          if (record.index - 1 !== currentEpisodeIndex) {
  ...
  const saveCurrentPlayProgress = async () => {
    if (!artPlayerRef.current || !currentSourceRef.current || !currentIdRef.current || !videoTitleRef.current || !detailRef.current?.source_name) return;
  ...
  useEffect(() => {
    if (!Artplayer || !Hls || !videoUrl || loading || currentEpisodeIndex === null || !artRef.current) return;
    ...
    if (artPlayerRef.current) cleanupPlayer();
    ...
    try {
      ...
      artPlayerRef.current = new Artplayer({
        ...
        settings: [
          ...
          },
          {
            ...
            tooltip: skipConfigRef.current.intro_time === 0 ? '设置片头时间' : `${formatTime(skipConfigRef.current.intro_time)}`,
            onClick: function () {
              const currentTime = artPlayerRef.current?.currentTime || 0;
              if (currentTime > 0) {
                // 点击设置时，自动将总开关打开
                handleSkipConfigChange({ ...skipConfigRef.current, intro_time: currentTime, enable: true });
          ...
          },
          {
            ...
            tooltip: skipConfigRef.current.outro_time >= 0 ? '设置片尾时间' : `-${formatTime(-skipConfigRef.current.outro_time)}`,
            onClick: function () {
              const outroTime = -(artPlayerRef.current?.duration - artPlayerRef.current?.currentTime) || 0;
              if (outroTime < 0) {
                // 点击设置时，自动将总开关打开
                handleSkipConfigChange({ ...skipConfigRef.current, outro_time: outroTime, enable: true });
      ...
      artPlayerRef.current.on('play', () => requestWakeLock());
      artPlayerRef.current.on('pause', () => { releaseWakeLock(); saveCurrentPlayProgress(); });

      if (artPlayerRef.current && !artPlayerRef.current.paused) requestWakeLock();

      artPlayerRef.current.on('video:volumechange', () => { lastVolumeRef.current = artPlayerRef.current.volume; });
      artPlayerRef.current.on('video:ratechange', () => { lastPlaybackRateRef.current = artPlayerRef.current.playbackRate; });
      ...
      artPlayerRef.current.on('video:canplay', () => {
        ...
        setTimeout(() => {
          ...
          if (Math.abs(artPlayerRef.current.playbackRate - lastPlaybackRateRef.current) > 0.01 && isWebkit) {
      ...
      artPlayerRef.current.on('video:timeupdate', () => {
        ...
        // 【防御 1】：挂起跳转期间的校验
        if (resumeTimeRef.current && resumeTimeRef.current > 0) return;

        const introTime = skipConfigRef.current.intro_time;
        const outroTime = skipConfigRef.current.outro_time; // 负数

        // 【防御 2】：HLS 幽灵锁 - 防止 currentTime 突然归零但真实进度已经越过了片头
        // 只有在视频已就绪、未在 seeking 时才执行跳过动作
        if (introTime > 0) {
          if (currentTime < introTime && !hasSkippedIntroRef.current) {
            if (player.video && !player.video.seeking && player.video.readyState >= 1) {
              player.currentTime = introTime;
              player.notice.show = `已自动跳过片头 (${formatTime(introTime)})`;
            }
          } else if (currentTime >= introTime && !hasSkippedIntroRef.current) {
            // 当真实播放进度越过片头后锁定标记，避免手动拖动回片头被弹飞
        ...
        if (outroTime < 0) {
          if (
            ...
            if (d && d.episodes && idx < d.episodes.length - 1 && !player.paused) {
              ...
              player.notice.show = `已自动跳过片尾 (${formatTime(outroTime)})，即将播放下一集`;
              player.pause();
              setCurrentEpisodeIndex(idx + 1);
              return;
            }
                ...
                {isVideoLoading && (
                  <div className='absolute inset-0 bg-black/85 backdrop-blur-sm rounded-xl flex items-center justify-center z-[500] transition-all duration-300'>
                    <div className='text-center max-w-md mx-auto px-6'>
                      ...
                      <div className='space-y-2'>
                        <p className='text-xl font-semibold text-white animate-pulse'>{videoLoadingStage === 'sourceChanging' ? '🔄 切换播放源...' : '🔄 视频加载中...'}</p>
                      </div>
                    </div>
                  </div>
                )}
