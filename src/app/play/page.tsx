...
function PlayPageClient() {
  ...
  useEffect(() => {
    ...
    const initAll = async () => {
      ...
      if (currentSource && currentId && !sourcesInfo.some((source) => source.source === currentSource && source.id === currentId)) {
      ...
      if (currentSource && currentId && !needPreferRef.current) {
        const target = sourcesInfo.find((source) => source.source === currentSource && source.id === currentId);
  ...
  const handleSourceChange = async (newSource: string, newId: string, newTitle: string) => {
    try {
      ...
      const newDetail = availableSources.find((source) => source.source === newSource && source.id === newId);
      if (!newDetail) {
        setError('未找到匹配结果');
        return;
      }
  ...
  // =========================================================================
  // 核心：播放器实例生命周期管理（完美解决 OOM 与幽灵声音问题）
  // =========================================================================
  useEffect(() => {
    ...
    const destroyPlayer = () => {
      if (artPlayerRef.current) {
        try {
          artPlayerRef.current.pause(); // 强制按下暂停键
        } catch (e) {}
        
        const videoElement = artPlayerRef.current.video;
        if (videoElement) {
          videoElement.removeAttribute('src'); // 强行切断流媒体解码
          videoElement.load(); // 强行让浏览器放弃旧媒体流
          if (videoElement.hls) {
            videoElement.hls.destroy(); // 销毁后台 WebWorker 下载线程
            delete videoElement.hls;
          }
        }
        artPlayerRef.current.destroy(true); // 连同 DOM 节点一并碾碎
    ...
    if (artPlayerRef.current) {
      if (!isWebkit) {
        ...
        // [关键闭包拦截] 换集时，也必须将销毁函数返回，防止组件突然卸载
    ...
    try {
      ...
      artPlayerRef.current.on('video:timeupdate', () => {
        ...
        if (skipIntroRef.current > 0 && currentTime < skipIntroRef.current && !hasSkippedIntroRef.current) {
          player.currentTime = skipIntroRef.current;
          hasSkippedIntroRef.current = true;
          player.notice.show = `已自动跳过片头 ${skipIntroRef.current} 秒`;
        }

        if (skipOutroRef.current > 0 && duration > 0 && currentTime >= duration - skipOutroRef.current && !hasSkippedOutroRef.current) {
          const d = detailRef.current;
          const idx = currentEpisodeIndexRef.current;
        ...
        <div className='flex items-center justify-center min-h-screen bg-transparent'>
          <div className='text-center max-w-md mx-auto px-6'>
            ...
            <div className='space-y-2'>
              <p className='text-xl font-semibold text-gray-800 dark:text-gray-200 animate-pulse'>{loadingMessage}</p>
            </div>
          </div>
        </div>
      </PageLayout>
        ...
        <div className='flex items-center justify-center min-h-screen bg-transparent'>
          <div className='text-center max-w-md mx-auto px-6'>
            ...
            <div className='space-y-3'>
              <button onClick={() => videoTitle ? router.push(`/search?q=${encodeURIComponent(videoTitle)}`) : router.back()} className='w-full px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl font-medium hover:from-green-600 hover:to-emerald-700 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl'>
                {videoTitle ? '🔍 返回搜索' : '← 返回上页'}
              </button>
              <button onClick={() => window.location.reload()} className='w-full px-6 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-xl font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200'>
                🔄 重新尝试
              </button>
            </div>
          </div>
